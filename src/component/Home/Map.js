...
function Map(props) {
  ...
  const initMap = async (data) => {
    ...
    data.map((item) => {
      ...
      const marker = {
        lat: parseFloat(item.lat_),
        lng: parseFloat(item.long_),
      };
      ...
      {isBrowser ? (
        <div className="DAT_MapContainer">
          <div className="DAT_MapContainer_title">
            <div className="DAT_MapContainer_title_name">
              {dataLang.formatMessage({ id: "map" })}
            </div>

            <div
              className="DAT_MapContainer_title_close"
              onClick={() => {
                props.handleClose();
              }}
              id="Popup-2"
          ...
          <div className="DAT_MapContainer_map" id="map"></div>
        </div>
      ) : (
        <div
          className="DAT_MapContainerMobile"
          style={{
            height: isLandscape ? "calc(100vh - 80px)" : "calc(100vh - 170px)",
          }}
        >
          <div className="DAT_MapContainerMobile_title">
            <div className="DAT_MapContainerMobile_title_name">
              {dataLang.formatMessage({ id: "map" })}
            </div>

            <div
              className="DAT_MapContainerMobile_title_close"
              onClick={() => {
                props.handleClose();
              }}
              id="Popup-2"
          ...
          <div className="DAT_MapContainerMobile_map" id="map"></div>
        </div>
      )}
    </>
  );
