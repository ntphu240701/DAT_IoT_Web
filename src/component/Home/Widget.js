  ...
  return (
    <div className="DAT_WidgetContainer">
      <div className="DAT_WidgetContainer_title">
        <div className="DAT_WidgetContainer_title_name">
          {dataLang.formatMessage({ id: "utilities" })}
        </div>

        <div
          className="DAT_WidgetContainer_title_close"
          onClick={() => {
            props.handleClose();
            setConfig("default");
          }}
      ...
      {(() => {
        switch (config) {
          case "device":
            return (
              <>
                <div className="DAT_WidgetContainer_titlesub">
                  <span>{dataLang.formatMessage({ id: "devicelist" })}</span>{" "}
                  <HiArrowUturnLeft
                ...
                <div className="DAT_WidgetContainer_list">
                  {props.logger.map((data, index) => {
                    return (
                      <div
                        className="DAT_WidgetContainer_list_item"
                        key={index}
                      >
                        <div className="DAT_WidgetContainer_list_item_index">
                          {index + 1}
                        </div>
                        <div
                          className="DAT_WidgetContainer_list_item_sn"
            ...
            return (
              <>
                <div className="DAT_WidgetContainer_data">
                  <div className="DAT_WidgetContainer_data_sn">
                    <span>SN: {widget.sn_}</span>{" "}
                    <CiEdit
                      size={20}
                      color="gray"
                      style={{ cursor: "pointer" }}
                      onClick={() => setConfig("device")}
                    />
                  </div>
                  <div className="DAT_WidgetContainer_data_screen">
                    <div className="DAT_WidgetContainer_data_screen_arrow">
                    ...
                    <div className="DAT_WidgetContainer_data_screen_name">
                      <MdScreenshotMonitor
                        ...
                        color="rgba(11, 25, 103)"
                      />
                      <div className="DAT_WidgetContainer_data_screen_name_config">
                        {" "}
                        <div className="DAT_WidgetContainer_data_screen_name_config_name">
                          {screenview.name_}
                        </div>
                        <CiEdit
                          size={20}
                ...
                <div className="DAT_WidgetContainer_status">
                  <input
                    ...
                    onChange={() =>
                      setWidget({
                        ...widget,
                        screenstate_: widget.screenstate_ === 1 ? 0 : 1,
                      })
                    }
                  />
                  {widget.screenstate_ === 1 ? (
                    <span style={{ color: "green" }}>
                      {dataLang.formatMessage({ id: "on" })}
                    </span>
                  ) : (
                    <span style={{ color: "red" }}>
                      {dataLang.formatMessage({ id: "off" })}
                    </span>
                  )}
                </div>
              </>
